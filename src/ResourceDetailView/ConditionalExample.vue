<template>
  ...
  -->
  <VCard title="Conditional Example" class="mb-2">
    <!-- 
    ...
    <VChipGroup class="px-2">
      ...
      <VChip
        ...
        >Server is on</VChip
      >
      <VChip v-else prepend-icon="mdi-alert-circle" size="small"
        >Server is off</VChip
      >
      ...
      >
      <VChip v-else prepend-icon="mdi-alert-circle" size="small"
        >User is not admin</VChip
      >

      <VChip
        :prepend-icon="
          datadogParam ? 'mdi-checkbox-marked-circle' : 'mdi-alert-circle'
        "
        size="small"
      >
        {{
    ...
    -->
    <BasicTextCard
      v-if="isServerOn && isUserAdmin && datadogParam"
      :title="datadogParam.label"
    >
      <template #default>
        <!-- 
...
/**
 * Creating a conditional example.
...
/**
 * This reactive value will check the props resource for the parameter properties
 * Custom Parameters can be created at `admin/customfields/` then added to the resource on the Parameters tab
 */
const params = computed(() =>
  props.resource?.groupedAttributes?.find(
    (attribute) => attribute.name === "parameters"
  )
);
/**
 * This reactive value will check the parameters for one whose name matches the desired value of "Datadog Parameter",
 * and then display this information in another VCard component
 */
const datadogParam = computed(
  () =>
    params.value?.attributes?.filter(
      (each) => each.label === "Datadog Parameter"
    )[0]
);
// This reactive value uses the already-provided resource prop to check if the server is powered on.
const isServerOn = computed(
  () => props.resource?.["server.powerStatus"] === "POWERON"
);
